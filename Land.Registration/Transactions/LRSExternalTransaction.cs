  ...
  public class LRSExternalTransaction {
    ...
    private void ApplyItemsRuleToTransaction(LRSTransaction transaction) {
      if (this.TransactionType.Id == 699 && this.DocumentType.Id == 708) {
        transaction.AddService(RecordingActType.Parse(2284), LRSLawArticle.Parse(874), BASE_SALARY_VALUE * 2);
        transaction.AddService(RecordingActType.Parse(2114), LRSLawArticle.Parse(859), BASE_SALARY_VALUE * 2);
      } else if (this.TransactionType.Id == 702 && this.DocumentType.Id == 713) {
        transaction.AddService(RecordingActType.Parse(2114), LRSLawArticle.Parse(859), BASE_SALARY_VALUE * 2);
      } else if (this.TransactionType.Id == 702 && this.DocumentType.Id == 710) {
        transaction.AddService(RecordingActType.Parse(2111), LRSLawArticle.Parse(859), BASE_SALARY_VALUE * 2);
      } else if (this.TransactionType.Id == 702 && this.DocumentType.Id == 711) {
        transaction.AddService(RecordingActType.Parse(2112), LRSLawArticle.Parse(859), BASE_SALARY_VALUE * 2);
      }
    }
